...
const Login = () => {
 ...
 const passwordResetStatus = params.get('passwordReset');
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // If this is the mobile/email field being modified
  ...
  const checkIsInstagramLoggedIn = (mobile: string) => {
    if (false) {
  ...
  return mobile.startsWith("+91") ? mobile.slice(3) : mobile;
    ...
    const loginPayload = {
      ...
      ...(formData.type === "mobile"
        ? { mobile: cleanMobileNumber(formData.mobile) }
        : { email: formData.email }),
    };

    try {
      const response = await dispatch(login(loginPayload)).unwrap();
      switch (response.code) {
        case 404:
          // toast.error('Password is not set for your account');
          showToast({
            message:
              "Password is not set for your account or incorrect email or phone number",
            type: "error",
            duration: 3000,
          });
          return false;
        case 402:
          // toast.error('Email and password do not match! Try again');
          showToast({
        ...
        case 406:
          ...
          return false;
        case 200:
          checkIsInstagramLoggedIn(
            formData.type === "mobile" ? formData.mobile : formData.email
          );
          router.push("/home");
          return true;
        default:
  ...
  useEffect(() => {
    ...
    const currentValue =
      formData.type === "email" ? formData.email : formData.mobile;

    if (formData.type === "email") {
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      setIsValid(emailRegex.test(currentValue));
    } else {
      setIsValid(currentValue.length === 10 && /^\d+$/.test(currentValue));
    }
  }, [formData.mobile, formData.email, formData.type]);

  const inputValue =
    formData.type === "email" ? formData.email : formData.mobile;

  return (
    <div className="max-w-[448px] h-screen overflow-scroll scroll-smooth [&::-webkit-scrollbar]:hidden bg-[#fff] mx-auto flex flex-col items-center justify-center">
      <div className="pt-6 w-[90%] mx-auto flex flex-col items-start justify-center gap-6">
        ...
        >
          <p className="m-0 text-[12px] font-normal leading-[145%]">
            Email or Mobile Number
          </p>

          <div className="flex flex-col w-full max-w-md mx-auto">
            <div className="flex items-center gap-2">
              <div className="flex items-center w-full placeholder:text-gray-400 border-[#dfe2e8] border-[1px] rounded-[9px] overflow-hidden">
                <input
                  id={formData.type === "email" ? "email" : "mobile"}
                  name={formData.type === "email" ? "email" : "mobile"}
                  type={"text"}
                  autoComplete={formData.type === "email" ? "email" : "tel"}
                  required
                  value={inputValue}
          ...
          <div className="flex flex-col items-end w-full max-w-md mx-auto">
            ...
            <p
              ...
              className="mt-1 text-[#000] text-center text-[12px] font-normal underline cursor-pointer "
            >
              Forgot Password?
            </p>
          </div>
          ...
          <div className="mx-auto w-full mt-7">
            ...
            mb-[8px] text-[18px] font-medium text-[#FFF] font-inter`}
              // onClick={handleSubmit}
            >
              Login
            </button>
            <p className="m-0 text-[#7e7e7e] text-[12px] font-normal leading-[145%]">
...
<Link href={'/Terms&Conditions'} target="_blank">
      ...
      <p className="mt-[20px] mb-[12px] text-[#333] text-[12px] font-inter font-medium ">
        OR
      </p>
      ...
      <div className="mx-auto w-[90%] ">
        <button
          // disabled={!isValid}
          className={`flex items-center justify-center py-3 w-full  rounded-[7px] bg-[#fff] border-[1px] border-[rgba(222,44,109,1)] cursor-pointer mb-[8px] text-[16px] font-medium text-[#000] font-inter`}
      ...
      <p
        onClick={() => router.push("/login/otp")}
        className="m-0 cursor-pointer mt-7 mb-5 text-[#000] text-[14px] font-bold text-center w-full leading-[145%]"
      >
        New to Hyyfam?{" "}
        <span className="text-[rgba(222,44,109,1)] text-[18px] underline ">
          Sign Up now.
        </span>
      </p>
    </div>
